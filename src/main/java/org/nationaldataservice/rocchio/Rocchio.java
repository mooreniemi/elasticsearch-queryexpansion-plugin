...
public class Rocchio {
    ...
    public static String NONEXISTENT_FIELD_ERROR(String index, String type, String field) {
        return "No field" +  field +" found on index " + index + " for: " + type;
    }
    ...
    private static Object digUpFieldValue(Map<String, Object> root, String field, Object fieldValue) {
        if (root.containsKey(field)) {
            fieldValue = root.get(field);
        }
        for (Map.Entry<String,Object> entry : root.entrySet())
            if (entry.getValue() instanceof Map<?, ?>) {
                fieldValue = digUpFieldValue((Map<String, Object>) entry.getValue(), field, fieldValue);
            }
    ...
    private String ensureTermVectors() throws IOException {
        ...
        // FIXME: not everything will have types now, right?
        MappingMetaData typeMetadata = indexMetaData.getMappings().get(type);
        Map<String, Object> typeMap = typeMetadata.getSourceAsMap();

        // NOTE: nested fields will be missing as keys
        String[] words = field.split("\\.");
        String normalizedField = words[words.length-1];
        Map<String, Object> fieldMap = (Map<String, Object>) digUpFieldValue(typeMap, normalizedField, null);
        if(fieldMap == null) {
            return NONEXISTENT_FIELD_ERROR(index, type, field);
        }

        // FIXME: does this handle multi-field?
        // Verify that "store" is present on target field
        if (fieldMap.containsKey("store")) {
            // Verify that term vector storage is enabled at the field level
            boolean storeEnabled = (boolean) fieldMap.get("store");
            if (!storeEnabled) {
                // WARN only, since this is a performance problem
     ...
     * @return FeatureVectorLocal based on feedback documents
     * @throws IOException
     *             if the TermVector has no fields, or if its Fields contain no terms
     */
    private FeatureVectorLocal getFeedbackVector(SearchHits hits, int fbDocs) throws IOException {
        FeatureVectorLocal summedDocVec = new FeatureVectorLocal(this.stopper);
        ...
        for (MultiTermVectorsItemResponse item : mtvresponse.getResponses()) {
            FeatureVectorLocal docVec = new FeatureVectorLocal(this.stopper);
        ...
        FeatureVectorLocal relDocTermVec = new FeatureVectorLocal(this.stopper);
     ...
     * @param query
     *            Query string
     * @return FeatureVectorLocal
     */
    public FeatureVectorLocal getQueryVector(String query) {
        // Create a query vector and scale by alpha
        FeatureVectorLocal rawQueryVec = new FeatureVectorLocal(this.stopper);
        rawQueryVec.addText(query);

        FeatureVectorLocal summedQueryVec = new FeatureVectorLocal(this.stopper);
        computeBM25Weights(rawQueryVec, summedQueryVec);

        FeatureVectorLocal queryTermVec = new FeatureVectorLocal(this.stopper);
    ...
    public FeatureVectorLocal expandQuery(String query, int fbDocs, int fbTerms) throws IOException {
        ...
        FeatureVectorLocal queryVector = getQueryVector(query);
    ...
    public static void main(String[] args) throws IOException, ParseException {
         ...
         * 
         * // Expand the query FeatureVectorLocal feedbackQuery = rocchio.expandQuery(query, fbDocs, fbTerms);
         * 
